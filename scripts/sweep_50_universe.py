"""
50-Ticker Full-Universe Strategy Sweep:
Systematically runs all 9 core strategy archetypes across all 50 tickers in DuckDB,
evaluates in-sample vs out-of-sample performance, and runs the Skeptic 5-test battery.

Usage:
    python scripts/sweep_50_universe.py
"""
import sys, os, time
import pandas as pd
import numpy as np
...
def run_universe_sweep():
    print(f"===========================================================", flush=True)
    print(f"  50-TICKER FULL-UNIVERSE QUANTITATIVE STRATEGY SWEEP     ", flush=True)
    print(f"===========================================================", flush=True)
    
    conn = init_db(config.DB_PATH)
    
    # Load all tickers
    tickers = [s for s in config.TICKERS if s != "SPY"] # SPY is market benchmark
    print(f"Evaluating {len(tickers)} symbols across multiple strategy families in {config.DB_PATH}...\n", flush=True)
    ...
    for symbol in tickers:
        ...
        for strat_name, strat_fn in strategy_funcs.items():
            ...
            # Count actual active signal days in OOS
            ...
            # Candidate criteria: OOS Sharpe >= 0.5, Max DD >= -30%, active trades >= 10
            is_candidate = (
                oos_sharpe >= 0.5 and 
                oos_dd >= -0.30 and 
            ...
            if verdict == "PASS" and is_candidate:
                ...
                print(f"  >>> [SKEPTIC PASS] {symbol:5s} | {strat_name:25s} | OOS Sharpe: {oos_sharpe:5.2f} | Total Ret: +{oos_tot_ret*100:5.1f}% | DD: {oos_dd*100:4.1f}% | Trades: {active_oos_trades:3d}", flush=True)
    ...
    print(f"\n===========================================================", flush=True)
    print(f"  SWEEP COMPLETED in {elapsed:.1f}s ({len(results)} backtests executed)", flush=True)
    print(f"===========================================================", flush=True)
    ...
    report_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "reports", "sweep_report_50_universe.txt")
    
    with open(report_path, "w") as f:
        f.write("=== 50-TICKER UNIVERSE STRATEGY SWEEP REPORT ===\n\n")
        f.write(f"Executed: {len(results)} experiments across {len(tickers)} symbols\n")
        ...
        f.write("\n\n--- Top 30 Candidates by Out-of-Sample Sharpe ---\n")
        f.write(res_df.sort_values(by="oos_sharpe", ascending=False).head(30).to_string(index=False))
        
    if skeptic_passes:
        pass_df = pd.DataFrame(skeptic_passes)[['symbol', 'strategy', 'oos_sharpe', 'active_oos_trades', 'oos_total_ret_pct', 'oos_max_dd_pct', 'skeptic_tests_passed']]
        print("--- SURVIVING ALPHA STRATEGIES (SKEPTIC CERTIFIED) ---")
        print(pass_df.sort_values(by="oos_sharpe", ascending=False).to_string(index=False))
    else:
        print("Top 15 candidates by out-of-sample Sharpe:")
        print(res_df.sort_values(by="oos_sharpe", ascending=False).head(15)[['symbol', 'strategy', 'oos_sharpe', 'active_oos_trades', 'oos_total_ret_pct', 'oos_max_dd_pct']].to_string(index=False))
